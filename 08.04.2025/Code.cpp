...
int main() {
    ...
    for (const auto& card : templateCards) {
        ...
        auto matchesFiltered = getBestMatches(rawMatches, 0.75);
...
std::vector<cv::DMatch> getBestMatches(const std::vector<std::vector<cv::DMatch>>& matches, double threshold = 0.75) {
...
void annotateMatch(cv::Mat& canvas, const CardTemplate& card, const std::vector<cv::Point2f>& region) {
    for (size_t i = 0; i < 4; ++i)
        cv::line(canvas, region[i], region[(i + 1) % 4], cv::Scalar(255, 0, 0), 2);  // Blue border

    cv::Point2f center(0, 0);
    for (const auto& pt : region) center += pt;
    center *= 1.0f / static_cast<float>(region.size());

    cv::putText(canvas, card.name, center - cv::Point2f(50, 0), cv::FONT_HERSHEY_DUPLEX, 0.8, cv::Scalar(0, 255, 0), 2);  // Green text
}
